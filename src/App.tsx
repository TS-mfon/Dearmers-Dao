...
function OnboardingGate({ children }: { children: React.ReactNode }) {
  ...
  const saveProfile = async () => { if (!handle.trim()) return; try { setSaving(true); const token = await getAccessToken(); if (!token || !user) throw new Error("Your Privy session expired. Sign in again."); const response = await fetch("/api/profile", { method: "POST", headers: { "content-type": "application/json", authorization: `Bearer ${token}` }, body: JSON.stringify({ identity: `privy:${user.id}`, email: user.email?.address || email, username: handle, displayName: handle, bio }) }); const body = await response.json() as { error?: string }; if (!response.ok) throw new Error(body.error || "Profile could not be created."); window.dispatchEvent(new Event("dearmers:profile-updated")); close(); navigate("/profile"); } catch (error) { window.alert(error instanceof Error ? error.message : "Profile could not be created."); } finally { setSaving(false); } };
