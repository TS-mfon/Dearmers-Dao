...
import { explainContractError, listDaos, metaMaskWalletClient, REGISTRY_ADDRESS, type DaoRecord } from "./lib/dao";
...
function App() {
  ...
  const connect = async (): Promise<Address | undefined> => {
    try {
      ...
      setNotice({ tone: "success", text: `MetaMask treasury connected: ${client.account!.address}` });
  ...
  return <BrowserRouter><OnboardingGate><ProductShell notice={notice} onSignOut={signOut}><Routes>
    <Route path="/" element={<SanctuaryLanding />} />
    <Route path="/sanctuary" element={<SanctuaryLanding />} />
    ...
    <Route path="/grants" element={<GrantExplorerRoute daos={daos} />} />
    <Route path="/grant-programs" element={<GrantExplorerRoute daos={daos} />} />
  ...
  </Routes></ProductShell></OnboardingGate>{busy && <div className="busy-overlay"><RefreshCw className="spin"/><strong>{busy}</strong><span>Confirm in MetaMask and keep this tab open.</span></div>}</BrowserRouter>;
}
...
function OnboardingGate({ children }: { children: React.ReactNode }) {
  ...
  const memberWallet = usePrivyMemberWallet();
  ...
  const saveProfile = async () => {
    ...
    try {
      setSaving(true);
      if (!memberWallet.authenticated) throw new Error("Sign in with Privy before creating your profile.");
      if (!memberWallet.address) throw new Error(memberWallet.error || (memberWallet.creating ? "Your Privy embedded wallet is still being prepared. Please wait a moment." : "Your Privy embedded wallet is not ready. Retry wallet setup."));
      const token = await getAccessToken();
      if (!token || !user) throw new Error("Your Privy session expired. Sign in again.");
      const payload = { identity: `privy:${user.id}`, email: user.email?.address || email, username: handle, displayName: handle, bio };
      let response: Response | undefined;
      let body: { error?: string } = {};
      for (let attempt = 0; attempt < 4; attempt += 1) {
        response = await fetch("/api/profile", { method: "POST", headers: { "content-type": "application/json", authorization: `Bearer ${token}` }, body: JSON.stringify(payload) });
        body = await response.json() as { error?: string };
        if (response.ok || response.status !== 409 || !/embedded wallet/i.test(body.error || "")) break;
        await new Promise((resolve) => window.setTimeout(resolve, 500 * (attempt + 1)));
      }
      if (!response?.ok) throw new Error(body.error || "Profile could not be created.");
      window.dispatchEvent(new Event("dearmers:profile-updated")); close(); navigate("/profile");
    } catch (error) { window.alert(error instanceof Error ? error.message : "Profile could not be created."); }
  ...
  return <>{children}{open && <div className="profile-onboarding-backdrop" role="presentation"><section className="profile-onboarding" role="dialog" aria-modal="true" aria-labelledby="profile-onboarding-title"><button className="profile-onboarding-close" onClick={close} aria-label="Close profile setup"><X size={18}/></button><div className="onboarding-sigil"><Sparkles size={18}/></div><span className="eyebrow">{currentStage === "auth" ? "FIRST ENTRY / AUTHENTICATE" : "SECOND ENTRY / PROFILE SIGNAL"}</span><h2 id="profile-onboarding-title">{currentStage === "auth" ? <>Enter the<br/><em>living network.</em></> : <>Make your presence<br/><em>legible to the network.</em></>}</h2><p>{currentStage === "auth" ? "Sign in with Privy. An embedded Ethereum wallet is created automatically for proposals, membership, and voting." : "Choose the name and signal other DAOs will recognise. You can refine this profile later."}</p>{currentStage === "auth" ? <div className="onboarding-options"><div className="onboarding-email"><span>Privy email login</span><input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="you@example.com" disabled={codeSent}/>{codeSent && <input inputMode="numeric" value={code} onChange={(event) => setCode(event.target.value)} placeholder="6-digit code"/>}<button type="button" disabled={!email || (codeSent && !code)} onClick={() => void authenticateEmail()}>{codeSent ? "Verify code" : "Send secure code"}</button>{authenticated && <button type="button" onClick={() => setStage("profile")}>Continue to profile</button>}</div></div> : <div className="onboarding-profile-form">{authenticated && !memberWallet.address && <div className="notice" role="status">{memberWallet.creating ? "Preparing your Privy embedded wallet…" : memberWallet.error ? <><span>{memberWallet.error}</span><button type="button" className="text-button" onClick={memberWallet.retry}>Retry wallet setup</button></> : "Finishing your Privy wallet setup…"}</div>}<label><span>Public handle</span><input value={handle} onChange={(event) => setHandle(event.target.value)} placeholder="e.g. mfon.builds" autoFocus/></label><label><span>Role / short bio</span><textarea value={bio} onChange={(event) => setBio(event.target.value)} placeholder="What are you building or contributing?" rows={4}/></label><button className="primary-button" disabled={saving || !handle.trim() || (authenticated && !memberWallet.address)} onClick={() => void saveProfile()}>{saving ? "Saving identity…" : "Create my profile"}<ArrowRight size={16}/></button></div>}<small className="onboarding-note">Privy handles authentication and your embedded member wallet. MetaMask is only required when creating a DAO treasury.</small></section></div>}</>;
}
