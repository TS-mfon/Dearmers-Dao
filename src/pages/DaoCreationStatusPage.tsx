...
export function DaoCreationStatusPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (body.indexed && body.daoId) navigate(`/dao/${body.daoId}`, { replace: true });
        ...
        const resumed = await fetch("/api/dao-creation", { method: "POST", headers: { ...(await headers()), "content-type": "application/json" }, body: JSON.stringify({ action: "resume", clientKey }) });
        if (!resumed.ok) { const reason = await resumed.json(); throw new Error(reason.error || "Could not resume DAO setup."); }
      }
  ...
  const failed = job.status === "needs_attention" || job.status === "failed" || Boolean(error);
  const complete = job.status === "ready" || job.indexed;
