...
async function repairLegacyGovernance(dao: Record<string, unknown>, address: Address, currentVersion: bigint): Promise<"ready" | "pending"> {
  ...
  try {
    const result = await client.readContract({ address, abi: daoAbi, functionName: "getConstitution", args: [currentVersion + 1n] });
  ...
  const scheduleTxHash = await signer.writeContract({ address, abi: daoAbi, functionName: "scheduleConstitution", args: [policy] });
