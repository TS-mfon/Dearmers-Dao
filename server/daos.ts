...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      const counts = new Map(memberCounts.map((entry) => [String(entry._id), Number(entry.count || 0)]));
      return json(res, 200, { daos: await Promise.all(records.map(async (record) => ({ ...await resolveDaoMedia(db, record), memberCount: counts.get(String(record.daoId || "")) || 0 }))) });
    }
