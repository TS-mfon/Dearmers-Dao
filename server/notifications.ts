...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (req.method === "GET") return json(res, 200, { notifications: await db.collection("notifications").find({ identity: identity.sub }).project({ identity: 0 }).sort({ createdAt: -1 }).limit(100).toArray() });
