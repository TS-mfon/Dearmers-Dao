...
export async function database(): Promise<Db> {
  ...
  indexesPromise ||= (async () => {
    ...
    await Promise.all([
      ...
      db.collection("delegations").dropIndex("creationKey_1").catch(() => undefined).then(() => db.collection("delegations").createIndex({ creationKey: 1, actor: 1 }, { unique: true, partialFilterExpression: { creationKey: { $type: "string" }, actor: { $type: "string" } } })),
      ...
      db.collection("grants").createIndex({ grantId: 1 }, { unique: true, partialFilterExpression: { grantId: { $type: "string" } } }),
