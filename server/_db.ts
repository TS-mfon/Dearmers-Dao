...
export async function database(): Promise<Db> {
  ...
  indexesPromise ||= (async () => {
    ...
    await Promise.all([
      ...
      db.collection("follows").dropIndex("follower_1_target_1").catch(() => undefined).then(() => db.collection("follows").createIndex({ actor: 1, target: 1 }, { unique: true })),
      db.collection("bookmarks").dropIndex("follower_1_target_1").catch(() => undefined).then(() => db.collection("bookmarks").createIndex({ actor: 1, target: 1 }, { unique: true })),
